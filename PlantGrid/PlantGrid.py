...
## For DG2F X0 = 746301.801, Y0 = 3382007.260
x0 = 746318.7099
y0 = 3381990.6059
...
## 4ft or 1.2192 m, for DG2F rangespace = 2.62
rangespace = 1.31
...
for rangenumber in range (0,ranges):
    for rownumber in range (0,rows):
        #create box coordinates
        x1 = x0-(cos(radbeta)*(rowwidth+rowspace)*rownumber)+(cos(radtheta)*(rowlen+rangespace)*rangenumber)
        y1 = y0+(sin(radbeta)*(rowwidth+rowspace)*rownumber)+(sin(radtheta)*(rowlen+rangespace)*rangenumber)

        x2 = x1+(cos(radtheta)*rowlen)
        y2 = y1+(sin(radtheta)*rowlen)
        ...
        #idtxt="{}.{}" 
        #feature.SetField('id', idtxt.format(rangenumber,rownumber))
